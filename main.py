...
def extract_structure_data(uploaded_files: List[str]) -> Dict[str, Any]:
    ...
    (for csv/json/image we call the small handlers).
    """
    structure = {}
    ...
    for filepath in uploaded_files:
        ...
        try:
            if ftype == "csv":
                ...
                # tools.get_image returns a Gemini description text; okay for structure
            ...
            content = f"Error reading {ftype} file: {str(e)}"
...
def make_session_dir():
    base_dir = os.environ.get("SESSION_BASE_DIR", tempfile.gettempdir())
...
async def process_questions_in_batches(
    ...
    for fp, content in structure.items():
        ...
        try:
            if ftype == "csv":
                # For CSV, preview first ~5 lines (header + 4 rows)
                if isinstance(content, dict) and content.get("success") and "content" in content:
                    lines = content["content"].splitlines()
                    snippet = "\n".join(lines[:5])
                    entry["csv_preview"] = snippet
                elif isinstance(content, str):
                    lines = content.splitlines()
                    snippet = "\n".join(lines[:5])
                    entry["csv_preview"] = snippet
                else:
                    entry["csv_preview"] = "N/A"
            elif ftype in {"json", "jsonl"}:
                if isinstance(content, str):
                    entry["json_summary"] = content.replace("\n", " ")[:200]
                else:
                    entry["json_summary"] = "N/A"
            elif ftype == "image":
                if isinstance(content, str):
                    entry["image_desc"] = content[:200]
                else:
                    entry["image_desc"] = "N/A"
            else:
                if isinstance(content, str):
                    entry["content_summary"] = content[:200].replace("\n", " ")
                else:
